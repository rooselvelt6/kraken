...
This guide covers the current Rust workspace under `rust/` and the `kraken` CLI binary. If you are brand new, make the doctor health check your first run: start `kraken`, then run `/doctor`.
...
`/doctor` is the built-in setup and preflight diagnostic. Once you have a saved session, you can rerun it with `./target/debug/kraken --resume latest /doctor`.
...
The CLI binary is available at `rust/target/debug/kraken` after a debug build. Make the doctor check above your first post-build step.
...
```bash
cd rust
./target/debug/kraken doctor --output-format json
```
...
Set up a new repository with `.kraken` config, `.kraken.json`, `.gitignore` entries, and a `CLAUDE.md` guidance file:

```bash
cd /path/to/your/repo
./target/debug/kraken init
```
...
```bash
./target/debug/kraken init --output-format json
```
...
**Why structured fields matter:** Krakens can detect per-artifact state (`created` vs `updated` vs `skipped`) without substring-matching human prose. Use the `created[]`, `updated[]`, and `skipped[]` arrays for conditional follow-up logic (e.g., only commit if files were actually created, not just updated).
...
```bash
cd rust
./target/debug/kraken prompt "summarize this repository"
```
...
```bash
cd rust
./target/debug/kraken "explain rust/crates/runtime/src/lib.rs"
```
...
```bash
cd rust
./target/debug/kraken --output-format json prompt "status"
```

### Inspect worker state

The `kraken state` command reads `.kraken/worker-state.json`, which is written by the interactive REPL or a one-shot prompt when a worker executes a task. This file contains the worker ID, session reference, model, and permission mode.

Prerequisite: You must run `kraken` (interactive REPL) or `kraken prompt <text>` at least once in the repository to produce the worker state file.

```bash
cd rust
./target/debug/kraken state
```

JSON mode:
```bash
./target/debug/kraken state --output-format json
```

If you run `kraken state` before any worker has executed, you will see a helpful error:
```
error: no worker state file found at .kraken/worker-state.json
...
These commands are available inside the interactive REPL (`kraken` with no args). They extend the assistant with workspace analysis, planning, and navigation features.
...
```bash
cd rust
./target/debug/kraken --model sonnet prompt "review this diff"
./target/debug/kraken --permission-mode read-only prompt "summarize Cargo.toml"
./target/debug/kraken --permission-mode workspace-write prompt "update README.md"
./target/debug/kraken --allowedTools read,glob "inspect the runtime crate"
```
...
`kraken` accepts two Anthropic credential env vars and they are **not interchangeable** — the HTTP header Anthropic expects differs per credential shape. Putting the wrong value in the wrong slot is the most common 401 we see.
...
**Why this matters:** if you paste an `sk-ant-*` key into `ANTHROPIC_AUTH_TOKEN`, Anthropic's API will return `401 Invalid bearer token` because `sk-ant-*` keys are rejected over the Bearer header. The fix is a one-line env var swap — move the key to `ANTHROPIC_API_KEY`. Recent `kraken` builds detect this exact shape (401 + `sk-ant-*` in the Bearer slot) and append a hint to the error message pointing at the fix.

**If you meant a different provider:** if `kraken` reports missing Anthropic credentials but you already have `OPENAI_API_KEY`, `XAI_API_KEY`, or `DASHSCOPE_API_KEY` exported, you most likely forgot to prefix the model name with the provider's routing prefix. Use `--model openai/gpt-4.1-mini` (OpenAI-compat / OpenRouter / Ollama), `--model grok` (xAI), or `--model qwen-plus` (DashScope) and the prefix router will select the right backend regardless of the ambient credentials. The error message now includes a hint that names the detected env var.

## Local Models

`kraken` can talk to local servers and provider gateways through either Anthropic-compatible or OpenAI-compatible endpoints. Use `ANTHROPIC_BASE_URL` with `ANTHROPIC_AUTH_TOKEN` for Anthropic-compatible services, or `OPENAI_BASE_URL` with `OPENAI_API_KEY` for OpenAI-compatible services.
...
cd rust
./target/debug/kraken --model "claude-sonnet-4-6" prompt "reply with the word ready"
```
...
cd rust
./target/debug/kraken --model "qwen2.5-coder" prompt "reply with the word ready"
```
...
cd rust
./target/debug/kraken --model "llama3.2" prompt "summarize this repository in one sentence"
```
...
cd rust
./target/debug/kraken --model "openai/gpt-4.1-mini" prompt "summarize this repository in one sentence"
```
...
cd rust
./target/debug/kraken --model "qwen/qwen-max" prompt "hello"
# or bare:
./target/debug/kraken --model "qwen-plus" prompt "hello"
```
...
`kraken` has three built-in provider backends. The provider is selected automatically based on the model name, falling back to whichever credential is present in the environment.
...
You can add custom aliases in any settings file (`~/.kraken/settings.json`, `.kraken/settings.json`, or `.kraken/settings.local.json`):
...
3. Otherwise, `kraken` checks which credential is set: `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` first, then `OPENAI_API_KEY`, then `XAI_API_KEY`.
...
The name "codex" appears in the Kraken Code ecosystem but it does **not** refer to OpenAI Codex (the code-generation model). Here is what it means in this project:

- **`oh-my-codex` (OmX)** is the workflow and plugin layer that sits on top of `kraken`. It provides planning modes, parallel multi-agent execution, notification routing, and other automation features. See [PHILOSOPHY.md](./PHILOSOPHY.md) and the [oh-my-codex repo](https://github.com/Yeachan-Heo/oh-my-codex).
- **`.codex/` directories** (e.g. `.codex/skills`, `.codex/agents`, `.codex/commands`) are legacy lookup paths that `kraken` still scans alongside the primary `.kraken/` directories.
- **`CODEX_HOME`** is an optional environment variable that points to a custom root for user-level skill and command lookups.

`kraken` does **not** support OpenAI Codex sessions, the Codex CLI, or Codex session import/export. If you need to use OpenAI models (like GPT-4.1), configure the OpenAI-compatible provider as shown above in the [OpenAI-compatible endpoint](#openai-compatible-endpoint) and [OpenRouter](#openrouter) sections.

## HTTP proxy support

`kraken` honours the standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` environment variables (both upper- and lower-case spellings are accepted) when issuing outbound requests to Anthropic, OpenAI-, and xAI-compatible endpoints. Set them before launching the CLI and the underlying `reqwest` client will be configured automatically.
...
cd rust
./target/debug/kraken prompt "hello via the corporate proxy"
```
...
- If a proxy URL cannot be parsed, `kraken` falls back to a direct (no-proxy) client so existing workflows keep working; double-check the URL if you expected the request to be tunnelled.

## Common operational commands

```bash
cd rust
./target/debug/kraken status
./target/debug/kraken sandbox
./target/debug/kraken agents
./target/debug/kraken mcp
./target/debug/kraken skills
./target/debug/kraken system-prompt --cwd .. --date 2026-04-04
```

## Session management

REPL turns are persisted under `.kraken/sessions/` in the current workspace.

```bash
cd rust
./target/debug/kraken --resume latest
./target/debug/kraken --resume latest /status /diff
```
...
1. `~/.kraken.json`
2. `~/.config/kraken/settings.json`
3. `<repo>/.kraken.json`
4. `<repo>/.kraken/settings.json`
5. `<repo>/.kraken/settings.local.json`

# Container-first kraken-code workflows
...
- `rust/crates/rusty-claude-cli/src/main.rs` exposes that state through the `kraken sandbox` / `cargo run -p rusty-claude-cli -- sandbox` report.
...
```bash
docker build -t kraken-code-dev -f Containerfile .
```

### Podman

```bash
podman build -t kraken-code-dev -f Containerfile .
```
...
docker run --rm -it \
  -v "$PWD":/workspace \
  -e CARGO_TARGET_DIR=/tmp/kraken-target \
  -w /workspace/rust \
  kraken-code-dev \
...
If you want to run `kraken` against a second checkout while keeping `kraken-code` itself mounted read-write:
...
- Running with `CARGO_TARGET_DIR=/tmp/kraken-target` avoids leaving container-owned `target/` artifacts in your bind-mounted checkout.

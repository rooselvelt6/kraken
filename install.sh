#!/usr/bin/env bash
# Kraken Code installer
#
# Detects the host OS, verifies the Rust toolchain (rustc + cargo),
# builds the `kraken` binary from the `rust/` workspace, and runs a
...
#   KRAKEN_BUILD_PROFILE=debug|release   same as --release toggle
#   KRAKEN_SKIP_VERIFY=1                 same as --no-verify
...
Environment overrides:
  KRAKEN_BUILD_PROFILE   debug | release
  KRAKEN_SKIP_VERIFY     set to 1 to skip verification
EOF
}
...
BUILD_PROFILE="${KRAKEN_BUILD_PROFILE:-debug}"
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}6. 'kraken' not found after install${COLOR_RESET}
     The binary lives at:
       rust/target/${BUILD_PROFILE}/kraken
...
step "Building the kraken workspace (${BUILD_PROFILE})"
...
KRAKEN_BIN="${RUST_DIR}/target/${BUILD_PROFILE}/kraken"

if [ ! -x "${KRAKEN_BIN}" ]; then
    error "Expected binary not found at ${KRAKEN_BIN}"
...
if [ "${SKIP_VERIFY}" = "1" ]; then
    warn "verification skipped (--no-verify or KRAKEN_SKIP_VERIFY=1)"
else
    info "running: kraken --version"
    if VERSION_OUT="$("${KRAKEN_BIN}" --version 2>&1)"; then
        ok "kraken --version -> ${VERSION_OUT}"
    else
        error "kraken --version failed:"
        printf '%s\n' "${VERSION_OUT}" 1>&2
        exit 1
    fi

    info "running: kraken --help (smoke test)"
    if "${KRAKEN_BIN}" --help >/dev/null 2>&1; then
        ok "kraken --help responded"
    else
        error "kraken --help failed"
        exit 1
    fi
fi
...
cat <<EOF
${COLOR_GREEN}Kraken Code is built and ready.${COLOR_RESET}

  Binary:  ${COLOR_BOLD}${KRAKEN_BIN}${COLOR_RESET}
...
Try it out:
  ...
  ${KRAKEN_BIN} prompt "summarize this repository"

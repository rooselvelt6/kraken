...
impl MetricsCollector {
    ...
    pub fn record_request(&mut self, provider: &str, latency_ms: u64, tokens: u64) {
        ...
        let metrics = self.providers.entry(provider.to_string()).or_insert_with(||
            ProviderMetrics {
        ...
        if let Some(latencies) = self.latencies.get(provider) {
            if let Some(metrics) = self.providers.get_mut(provider) {
                ...
                if len > 0 {
                    ...
                    metrics.p95_latency_ms = sorted[(len as f64 * 0.95) as usize].min(sorted[len - 1]);
                    metrics.p99_latency_ms = sorted[(len as f64 * 0.99) as usize].min(sorted[len - 1]);
                }
            }
        }
    }
...
impl MetricsCollectorSync {
    ...
    pub fn error_rate(&self) -> f64 {
        self.inner.lock().ok().map(|i| i.error_rate()).unwrap_or(0.0)
    }
}

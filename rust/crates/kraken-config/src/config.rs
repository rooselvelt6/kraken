...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".kraken.json"),
            |parent| parent.join(".kraken.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".kraken.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".kraken").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".kraken").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("KRAKEN_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".kraken")))
        .unwrap_or_else(|| PathBuf::from(".kraken"))
}
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".kraken.json");
...
mod tests {
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".kraken");
        fs::create_dir_all(cwd.join(".kraken")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".kraken.json"),
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".kraken");
        let project_settings = cwd.join(".kraken").join("settings.json");
        fs::create_dir_all(cwd.join(".kraken")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

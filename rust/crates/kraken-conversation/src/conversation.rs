...
use kraken_session::compact::{
    compact_session, estimate_session_tokens, CompactionConfig, CompactionResult,
};
use kraken_config::config::RuntimeFeatureConfig;
use kraken_session::hooks::{HookAbortSignal, HookProgressReporter, HookRunResult, HookRunner};
use kraken_policy::permissions::{
    PermissionContext, PermissionOutcome, PermissionPolicy, PermissionPrompter,
};
use kraken_session::session::{ContentBlock, ConversationMessage, Session};
use kraken_session::usage::{TokenUsage, UsageTracker};
...
mod tests {
    ...
    use kraken_session::compact::CompactionConfig;
    use kraken_config::config::{RuntimeFeatureConfig, RuntimeHookConfig};
    use kraken_policy::permissions::{
        PermissionMode, PermissionPolicy, PermissionPromptDecision, PermissionPrompter,
        PermissionRequest,
    };
    use kraken_session::prompt::{ProjectContext, SystemPromptBuilder};
    use kraken_session::session::{ContentBlock, MessageRole, Session};
    use kraken_session::usage::TokenUsage;
    use crate::ToolError;
    use std::fs;
    ...
    fn reconstructs_usage_tracker_from_restored_session() {
        ...
        session
            .messages
            .push(kraken_session::session::ConversationMessage::assistant_with_usage(
    ...
    fn auto_compacts_when_cumulative_input_threshold_is_crossed() {
        ...
        session.messages = vec![
            kraken_session::session::ConversationMessage::user_text("one"),
            kraken_session::session::ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "two".to_string(),
            }]),
            kraken_session::session::ConversationMessage::user_text("three"),
            kraken_session::session::ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "four".to_string(),
            }]),
        ];

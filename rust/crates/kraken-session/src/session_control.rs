...
pub struct SessionStore {
    ...
    /// `/home/user/project/.kraken/sessions/a1b2c3d4e5f60718/`.
...
impl SessionStore {
    ...
    ///
    /// The on-disk layout becomes `<cwd>/.kraken/sessions/<workspace_hash>/`.
...
fn format_missing_session_reference(reference: &str, sessions_root: &Path) -> String {
    // #80: show the actual workspace-fingerprint directory instead of lying about .kraken/sessions/
    let fingerprint_dir = sessions_root
        .file_name()
        .and_then(|f| f.to_str())
        .unwrap_or("<unknown>");
    format!(
        "session not found: {reference}\nHint: managed sessions live in .kraken/sessions/{fingerprint_dir}/ (workspace-specific partition).\nTry `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}
...
fn format_no_managed_sessions(sessions_root: &Path) -> String {
    ...
    let fingerprint_dir = sessions_root
        ...
        .unwrap_or("<unknown>");
    format!(
        "no managed sessions found in .kraken/sessions/{fingerprint_dir}/\nStart `kraken` to create a session, then rerun with `--resume {LATEST_SESSION_REFERENCE}`.\nNote: kraken partitions sessions per workspace fingerprint; sessions from other CWDs are invisible."
    )
}
...
mod tests {
    ...
    fn session_store_rejects_legacy_session_from_other_workspace() {
        ...
        let store_b = SessionStore::from_cwd(&workspace_b).expect("store b should build");
        let legacy_root = workspace_b.join(".kraken").join("sessions");
    ...
    fn session_store_loads_safe_legacy_session_from_same_workspace() {
        ...
        let store = SessionStore::from_cwd(&base).expect("store should build");
        let legacy_root = base.join(".kraken").join("sessions");

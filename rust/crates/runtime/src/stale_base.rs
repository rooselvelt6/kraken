...
/// Read the `.kraken-base` file from the given directory and return the trimmed
/// commit hash, or `None` when the file is absent or empty.
pub fn read_kraken_base_file(cwd: &Path) -> Option<String> {
    let path = cwd.join(".kraken-base");
...
/// fall back to reading `.kraken-base` from `cwd`.
pub fn resolve_expected_base(flag_value: Option<&str>, cwd: &Path) -> Option<BaseCommitSource> {
    ...
    read_kraken_base_file(cwd).map(BaseCommitSource::File)
}
...
mod tests {
    ...
    #[test]
    fn reads_kraken_base_file() {
        // given
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".kraken-base"), "abc1234def5678\n").expect("write .kraken-base");

        // when
        let value = read_kraken_base_file(&root);
    ...
    #[test]
    fn returns_none_for_missing_kraken_base_file() {
        // given
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
    ...
    #[test]
    fn returns_none_for_empty_kraken_base_file() {
        // given
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".kraken-base"), "  \n").expect("write empty .kraken-base");
    ...
    fn resolve_expected_base_prefers_flag_over_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".kraken-base"), "from_file\n").expect("write .kraken-base");
    ...
    #[test]
    fn matches_with_kraken_base_file_in_real_repo() {
        // given
        let root = temp_dir();
        init_repo(&root);
        let sha = head_sha(&root);
        fs::write(root.join(".kraken-base"), format!("{sha}\n")).expect("write .kraken-base");
        let source = resolve_expected_base(None, &root);
    ...
    #[test]
    fn diverged_with_kraken_base_file_after_new_commit() {
        // given
        let root = temp_dir();
        init_repo(&root);
        let old_sha = head_sha(&root);
        fs::write(root.join(".kraken-base"), format!("{old_sha}\n")).expect("write .kraken-base");

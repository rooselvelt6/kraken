...
/// Write current worker state to `.kraken/worker-state.json` under the worker's cwd.
/// This is the file-based observability surface: external observers (krakenhip, orchestrators)
...
fn emit_state_file(worker: &Worker) {
    let state_dir = std::path::Path::new(&worker.cwd).join(".kraken");
...
mod tests {
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".kraken").join("worker-state.json");
        assert!(
            state_path.exists(),
    ...
    fn startup_timeout_classifies_trust_required_when_prompt_blocked() {
        ...
        let timed_out = registry
            .observe_startup_timeout(&worker.worker_id, "kraken prompt", true, true)
            .expect("startup timeout observe should succeed");

...
const GITIGNORE_COMMENT: &str = "# Kraken Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 3] = [
    ".kraken/settings.local.json",
    ".kraken/sessions/",
    ".krakenhip/",
];
...
impl InitStatus {
    ...
    /// Unlike `label()`, this never changes wording: krakens can switch on
...
impl InitReport {
    ...
    /// Summary constant that krakens can embed in JSON output without having
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let kraken_dir = cwd.join(".kraken");
    artifacts.push(InitArtifact {
        name: ".kraken/",
        status: ensure_dir(&kraken_dir)?,
    });

    let kraken_json = cwd.join(".kraken.json");
    artifacts.push(InitArtifact {
        name: ".kraken.json",
        status: write_file_if_missing(&kraken_json, STARTER_KRAKEN_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Kraken Code (krakencode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.kraken.json`; reserve `.kraken/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".kraken.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".kraken").is_dir());
        assert!(root.join(".kraken.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".kraken.json")).expect("read kraken json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".kraken/settings.local.json"));
        assert!(gitignore.contains(".kraken/sessions/"));
        assert!(gitignore.contains(".krakenhip/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".kraken/settings.local.json\n")
        ...
        assert!(second_rendered.contains(".kraken/"));
        assert!(second_rendered.contains(".kraken.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".kraken/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".kraken/sessions/").count(), 1);
        assert_eq!(gitignore.matches(".krakenhip/").count(), 1);
